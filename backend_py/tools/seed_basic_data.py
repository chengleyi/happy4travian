...
def seed_users_accounts_villages(db):
    ...
    if not s:
        ...
        db.add(s)
        db.flush()
    rom = db.execute(select(Tribe).where(Tribe.code=="ROM").limit(1)).scalar_one_or_none()
    if not rom:
        rom = Tribe(code="ROM", name="Romans")
        db.add(rom)
        db.flush()
    ...
    teu = db.execute(select(Tribe).where(Tribe.code=="TEU").limit(1)).scalar_one_or_none()
    if not teu:
        teu = Tribe(code="TEU", name="Teutons")
        db.add(teu)
        db.flush()
    gal = db.execute(select(Tribe).where(Tribe.code=="GAL").limit(1)).scalar_one_or_none()
    if not gal:
        gal = Tribe(code="GAL", name="Gauls")
        db.add(gal)
        db.flush()
...
def seed_alliance(db):
    ...
    if not a1:
        ...
        db.add(a1)
        db.flush()
    acc1 = db.execute(select(GameAccount).where(GameAccount.in_game_name=="player1").limit(1)).scalar_one_or_none()
    ...
    acc2 = db.execute(select(GameAccount).where(GameAccount.in_game_name=="player2").limit(1)).scalar_one_or_none()
    ...
    if not a2:
        ...
        db.add(a2)
        db.flush()
    acc3 = db.execute(select(GameAccount).where(GameAccount.in_game_name=="player3").limit(1)).scalar_one_or_none()
    ...
    acc4 = db.execute(select(GameAccount).where(GameAccount.in_game_name=="player4").limit(1)).scalar_one_or_none()
